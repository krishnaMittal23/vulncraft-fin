 ...
 * @param {string} [userId] - prefer this user's saved OpenRouter key if present
...
async function chat(message, userId) {
  ...
  const openai = await getOpenRouterClient(userId, { timeout: 30000 });
...
- web-hygiene: Security headers, TLS/cert health, cookie flags, CORS, and exposed files (.git/.env/source maps). Fast; great for modern sites.
- nuclei: Template-based scanner for CVEs, exposures, and misconfigurations.
- js-recon: Parse a modern site's JavaScript bundles for API endpoints, URLs, and leaked secrets (best for SPAs).
- code-scan: SAST + secret-scan + dependency-audit on a GitHub repo's source (Semgrep, gitleaks, osv-scanner). Use for code/repo workflows — no deployment needed.
- nmap: Port scanning and network discovery
- nikto: Web server scanner
- sqlmap: SQL injection testing
- gobuster: Directory/file brute forcing
- wpscan: WordPress security scanner
- owasp-zap: OWASP ZAP comprehensive web app scanner (spiders + active-scans discovered routes)
...
Optional per-node config (put these keys directly in the node's "data", omit for defaults):
- nmap: {"nmap_arguments": "-sV -F"} (flags are server-side allow-listed)
- sqlmap: {"sqlmap_level": 1-5, "sqlmap_risk": 1-3}
- gobuster: {"gobuster_extensions": "php,js,html"}
- wpscan: {"wpscan_enumerate": "vp,vt,u"}
- nuclei: {"severity": "low,medium,high,critical", "timeout": 240}
- web-hygiene: {"check_exposed_paths": true}
- owasp-zap: {"active_scan": true, "spider": true, "timeout": 300}
...
Example prompts:
- "scan example.com for vulnerabilities" → web-hygiene → nuclei → email
- "full web security scan" → web-hygiene → nmap → nuclei → owasp-zap → sqlmap → email
- "check for SQL injection" → sqlmap → email
- "WordPress security check" → wpscan → email
- "quick hygiene + CVE check" → web-hygiene → nuclei → slack
...
Return ONLY the JSON, no explanations.`;
  ...
  try {
    const response = await openai.chat.completions.create({
        model: "anthropic/claude-haiku-4-5", // More generous rate limits
      messages: [

...
// Client options applied to per-request OpenRouter clients
 ...
 */
async function analyzeCode(codeSnippet, language = "", userId) {
  ...
  try {
    const openai = await getOpenRouterClient(userId, CLIENT_OPTS);
 ...
 */
async function processChunk(chunk, question, chunkIndex, totalChunks, openai) {
 ...
 */
async function getQueryAboutCode(codeFiles, question = "", userId) {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        const chunkResponse = await processChunk(chunks[i], question, i, chunks.length, openai);

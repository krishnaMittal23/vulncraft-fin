...
async function makeLLMRequest(systemPrompt, userPrompt, attemptNumber = 0) {
  ...
  try {
    const client = clientStore.getStore() || (await getOpenRouterClient());
 ...
 */
async function generateDetailedNodeReport(nodeType, nodeData, userId) {
  console.log(`\n📊 Generating detailed report for ${nodeType} node...`);

  // Use the calling user's saved OpenRouter key when available (env fallback).
  const client = await getOpenRouterClient(userId, { maxRetries: 2 });

  return clientStore.run(client, async () => {
    ...
    try {
      switch (nodeType) {
        ...
        case "web-hygiene":
          return await analyzeDataInChunks(nodeData, nodeType, analyzeWebHygieneResults);

        case "nuclei":
          return await analyzeDataInChunks(nodeData, nodeType, analyzeNucleiResults);
...
/**
 * Generate detailed analysis for Web Hygiene scan results
 */
async function analyzeWebHygieneResults(data, chunkIndex = null, totalChunks = null) {
  console.log("🧼 Analyzing Web Hygiene scan results...");

  const chunkInfo = chunkIndex ? ` (Chunk ${chunkIndex}/${totalChunks})` : '';

  const prompt = `You are a web security expert analyzing the HTTP/TLS posture of a website. Analyze the following Web Hygiene scan data${chunkInfo} (security headers, TLS/cert health, cookie flags, CORS policy, and exposed sensitive files) and provide a detailed security assessment.

Web Hygiene Scan Data:
${JSON.stringify(data, null, 2)}

Provide a detailed report in the following JSON format:
{
  "summary": "Brief overview of the site's web hygiene (2-3 sentences)",
  "findings": [
    {
      "title": "issue title",
      "severity": "HIGH|MEDIUM|LOW",
      "category": "headers|tls|cookies|cors|exposed-file|other",
      "description": "what the issue is and why it matters",
      "recommendation": "concrete remediation step"
    }
  ],
  "tls_assessment": "summary of TLS/certificate posture",
  "overallRisk": "HIGH|MEDIUM|LOW",
  "criticalFindings": ["most important issues"],
  "recommendations": ["prioritized recommendations"]
}

Focus on:
- Missing or weak security headers (HSTS, CSP, etc.) and their real-world impact
- TLS protocol/cipher/certificate problems
- Insecure cookie flags
- Permissive/reflected CORS
- Publicly exposed sensitive files (.git, .env, source maps, backups)

Return only valid JSON, no additional text.`;

  try {
    return await makeLLMRequest(
      "You are a web security expert specializing in HTTP/TLS hardening and configuration review.",
      prompt
    );
  } catch (error) {
    console.error("Error analyzing Web Hygiene results:", error.message);
    return {
      summary: "Analysis failed due to rate limiting or API error. Raw data available in report.",
      error: error.message,
      rawData: data
...
/**
 * Generate detailed analysis for Nuclei scan results
 */
async function analyzeNucleiResults(data, chunkIndex = null, totalChunks = null) {
  console.log("☢️ Analyzing Nuclei scan results...");

  const chunkInfo = chunkIndex ? ` (Chunk ${chunkIndex}/${totalChunks})` : '';

  const prompt = `You are a vulnerability analyst reviewing Nuclei template-based scan results. Analyze the following Nuclei findings${chunkInfo} and provide a detailed security assessment.
...
Provide a detailed report in the following JSON format:
{
  ...
  "vulnerabilities": [
    {
      "name": "finding/template name",
      "severity": "CRITICAL|HIGH|MEDIUM|LOW|INFO",
      "cve": "CVE id if present",
      "matchedAt": "URL/location where matched",
      "impact": "potential impact",
...
Focus on:
- Known CVEs and their exploitability
- Exposed panels, configs, and information disclosure
- Misconfigurations and default credentials
- Prioritization by severity and real-world risk

Return only valid JSON, no additional text.`;

  try {
    return await makeLLMRequest(
      "You are a vulnerability analyst specializing in triaging automated scanner output.",
      prompt
    );
  } catch (error) {
    console.error("Error analyzing Nuclei results:", error.message);
    return {
      summary: "Analysis failed due to rate limiting or API error. Raw data available in report.",
      error: error.message,
      rawData: data
...
module.exports = {
  ...
  analyzeWebHygieneResults,
  analyzeNucleiResults,
  analyzeNmapResults,

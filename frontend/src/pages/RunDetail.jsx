...
const nodeFindings = (r) => {
  if (!r) return [];
  if (r.output && Array.isArray(r.output.findings)) return r.output.findings;
  if (r.detailedAnalysis && Array.isArray(r.detailedAnalysis.vulnerabilities)) return r.detailedAnalysis.vulnerabilities;
  if (Array.isArray(r.vulnerabilities)) return r.vulnerabilities;
  if (r.data && Array.isArray(r.data.vulnerabilities)) return r.data.vulnerabilities;
  return [];
};
...
const RunDetail = () => {
  ...
  const timeline = useMemo(() => {
    ...
    const completedLive = new Map((progress?.completedNodes || []).map((n) => [n.nodeId, n]));
    ...
    const errByNode = new Map((report?.executionErrors || []).map((e) => [e.nodeId, e.message]));
    ...
    return planned.map((n) => {
      ...
      else if (completedLive.has(n.id)) status = (completedLive.get(n.id)).status;
      // The run h and this node h result → it never ran.
  ...
  const nodeLogs = useMemo(
    () => allLogs.filter((l) => selectedNode && l.nodeType === selectedNode.nodeType),
    ...
    <div className="min-h-screen bg-[#09090b] text-[#e5e1e4] flex flex-col" style={GEIST}>
      ...
      <header className="border-b border-[#27272a] px-6 py-4">
        ...
        <div className="mt-4 flex items-center gap-4">
          <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold ${SEV[riskSev].chip}`} style={MONO}>RISK</span>
            ...
            <div className="min-w-0">
              ...
              {(report.executionErrors || []).length > 0 ? (
                <ul className="mt-1 space-y-0.5 text-xs text-[#ef4444]/80" style={MONO}>
                  {report.executionErrors.slice(0, 5).map((e, i) => (
        ...
        <aside className="w-64 shrink-0 border-r border-[#27272a] overflow-y-auto p-3">
          {report.summary && (
            ...
            >
              <div className="flex items-center gap-2">
                <FileText className={`h-4 w-4 shrink-0 ${selected === "__summary__" ? "text-[#4be277]" : "text-[#bccbb9]"}`} />
          ...
          <div className="space-y-1">
            {timeline.map((t, i) => {
              ...
              const worst = t.findings.reduce((acc, f) => {
              ...
              {report.summary.remediation?.length > 0 && (
                <section>
                  ...
                  <div className="space-y-2">
                    {report.summary.remediation.map((r, i) => (
              ...
              <section>
                ...
                {(report.summary.findings || []).length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {report.summary.findings.map((f, i) => (

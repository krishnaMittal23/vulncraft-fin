...
const fontGeist = { fontFamily: "'Geist', sans-serif" } ;
const fontMono = { fontFamily: "'JetBrains Mono', monospace" } ;

const statusChipStyles = {
  running: {
    cls: "border-[#4be277]/40 text-[#4be277]",
    dot: "bg-[#4be277]",
    label: "RUNNING",
  },
  completed: {
    cls: "border-[#6ee7b7]/40 text-[#6ee7b7]",
    dot: "bg-[#6ee7b7]",
    label: "COMPLETED",
  },
  idle: {
    cls: "border-[#3f3f46] text-[#bccbb9]",
    dot: "bg-[#3f3f46]",
    label: "IDLE",
  },
  failed: {
    cls: "border-[#ef4444]/40 text-[#f87171]",
    dot: "bg-[#ef4444]",
    label: "FAILED",
  },
};
...
const Workflow = () => {
  ...
  const handleDownloadLogs = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/reports/${progress.reportId}/logs/download`, { headers: { ...authHeaders() } });
  ...
  const handleDownloadLogsZip = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/reports/${progress.reportId}/logs/download-zip`, { headers: { ...authHeaders() } });
    ...
    if (confirm(`Are you sure you want to delete "${name}"?`)) {
      try {
        ...
        toast.success("Workflow deleted", {
          description: `${name} h deleted successfully.`,
        });
      } catch (error) {
  ...
  const handleExecuteWorkflow = async (id, name, e) => {
    e.stopPropagation();
    try {
  ...
  // executed h live status (driven by the socket `progress`); others are idle.
  const cardStatusFor = (id)=> {
  ...
  const eyebrowFor = (executionMode) =>
    `// ${(executionMode || "PIPELINE").toUpperCase()}`;

  const relativeUpdated = (date) => {
    try {

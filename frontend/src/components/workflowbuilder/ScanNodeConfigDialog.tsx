...
interface ScanNodeConfigDialogProps {
  open: boolean;
...
const NUCLEI_SEVERITIES = ["low", "medium", "high", "critical"] as const;
...
const ScanNodeConfigDialog = ({
  ...
  initialData,
  onSave,
}: ScanNodeConfigDialogProps) => {
  // nmap
  const [nmapProfile, setNmapProfile] = useState<string>(initialData?.nmap_profile || "service");
  const [nmapPorts, setNmapPorts] = useState<string>(initialData?.nmap_ports || "");
  // gobuster
  const [gobusterExtensions, setGobusterExtensions] = useState<string>(initialData?.gobuster_extensions || "");
  // sqlmap
  const [sqlmapLevel, setSqlmapLevel] = useState<string>(initialData?.sqlmap_level?.toString() || "1");
  const [sqlmapRisk, setSqlmapRisk] = useState<string>(initialData?.sqlmap_risk?.toString() || "1");
  const [testUrl, setTestUrl] = useState<string>(initialData?.testUrl || "");
  // wpscan
  const [wpscanEnumerate, setWpscanEnumerate] = useState<string>(initialData?.wpscan_enumerate || "vp,vt,u");
  // nikto
  const [niktoTuning, setNiktoTuning] = useState<string>(initialData?.nikto_tuning || "");
  // web-hygiene
  const [checkExposedPaths, setCheckExposedPaths] = useState<boolean>(
  ...
  const [nucleiTimeout, setNucleiTimeout] = useState<string>(initialData?.timeout?.toString() || "240");
  ...
  const handleSave = () => {
    ...
    switch (nodeType) {
      case "nmap": {
        const base = NMAP_PROFILES[nmapProfile] || "-F";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[460px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Configure {TITLES[nodeType] || nodeType} Node</DialogTitle>
          <DialogDescription>
          ...
          {nodeType === "nmap" && (
            <>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">Digits, commas and dashes only.</p>
              </div>
            </>
          )}
          ...
          {nodeType === "web-hygiene" && (
            <div className="flex items-start gap-3 rounded-md border p-3">
              <Checkbox
                ...
                onCheckedChange={(c: boolean) => setCheckExposedPaths(c)}
              />

    ...
    try:
        ...
        try:
            if data.get('run_nikto', False):
                ...
                nikto_result = run_nikto(target_url)
            ...
            if data.get('run_sqlmap', False) and data.get('test_url'):
                ...
                sqlmap_result = run_sqlmap(data.get('test_url'))
            ...
            if data.get('run_wpscan', False):
                ...
                if is_wordpress_site(target_url):
                    ...
                    wpscan_result = run_wpscan(target_url)
...
def run_nikto(url):
    ...
    try:
        ...
        # Try to run Nikto
        nikto_result = _execute_nikto(normalized_url)
        ...
        if nikto_result.get('error') and try_http_fallback and normalized_url.startswith('https://'):
            ...
            nikto_result = _execute_nikto(http_url)
...
def run_sqlmap(url):
    ...
    try:
        ...
        cmd = [
            ...
            "--level=1",  # Minimum level for faster scan
            "--risk=1",  # Minimum risk
...
def run_wpscan(url):
    ...
    try:
        ...
        cmd = [
            ...
            "--url", url,
            "--format", "json",
            "--api-token", "eNUgbbtp3MN0jnoptlAxGqny08nEMNYNnH4RzjssTwM",
            "--no-banner",
            "--random-user-agent",
            "--disable-tls-checks",
            "--enumerate", "vp,vt,u",  # Enumerate vulnerable plugins, themes, and users
